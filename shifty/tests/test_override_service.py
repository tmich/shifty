...
from shifty.domain.entities import Override, Shift
...
def test_take_success(service, mock_repository):
    override_id = uuid4()
    taken_by_id = uuid4()
    expected = make_override()
    mock_repository.get_by_id.return_value = expected
    mock_repository.take.return_value = expected
    result = service.take(override_id, taken_by_id)
    assert result == expected
    mock_repository.take.assert_called_once_with(override_id, taken_by_id)

def test_take_not_available(service, mock_repository):
    override_id = uuid4()
    taken_by_id = uuid4()
    expected = make_override()
    mock_repository.get_by_id.return_value = expected
    mock_repository.take.side_effect = ValueError
    with pytest.raises(ValueError):
        service.take(override_id, taken_by_id)
...
def test_take_by_shift_owner_forbidden(service, mock_repository, mock_shift_repository):
    ...
    with pytest.raises(Exception) as excinfo:
        service.take(override_id, shift_owner_id)
